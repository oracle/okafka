...
public class RecordAccumulatorTest {
    ...
    public void testFull() throws Exception {
        ...
        List<ProducerBatch> batches = accum.drain(cluster, Collections.singleton(node1), Integer.MAX_VALUE, 0).get(node1);
    ...
    public void testLinger() throws Exception {
        ...
        time.sleep(10);
        assertEquals("Our partition's leader should be ready", Collections.singleton(node1), accum.ready(cluster, time.milliseconds()).readyNodes);
        List<ProducerBatch> batches = accum.drain(cluster, Collections.singleton(node1), Integer.MAX_VALUE, time.milliseconds() + lingerMs + 1).get(node1);
        assertEquals(1, batches.size());
        ProducerBatch batch = batches.get(0);
    ...
    public void testPartialDrain() throws Exception {
        ...
        List<ProducerBatch> batches = accum.drain(cluster, Collections.singleton(node1), 1024, 0).get(node1);
    ...
    public void testStressfulSituation() throws Exception {
        ...
        while (read < numThreads * msgs) {
            ...
            List<ProducerBatch> batches = accum.drain(cluster, nodes, 5 * 1024, 0).get(node1);
            if (batches != null) {
    ...
    public void testRetryBackoff() throws Exception {
        ...
        assertEquals("Node1 should be ready", Collections.singleton(node1), result.readyNodes);
        Map<Node, List<ProducerBatch>> batches = accum.drain(cluster, result.readyNodes, Integer.MAX_VALUE, now + lingerMs + 1);
        assertEquals("Node1 should be the only ready node.", 1, batches.size());
        assertEquals("Partition 0 should only have one batch drained.", 1, batches.get(node1).size());

        // Reenqueue the batch
        now = time.milliseconds();
        accum.reenqueue(batches.get(node1).get(0), now);
        ...
        assertEquals("Node1 should be the only ready node.", 1, batches.size());
        assertEquals("Node1 should only have one batch drained.", 1, batches.get(node1).size());
        assertEquals("Node1 should only have one batch for partition 1.", tp2, batches.get(node1).get(0).topicPartition);
        ...
        assertEquals("Node1 should be the only ready node.", 1, batches.size());
        assertEquals("Node1 should only have one batch drained.", 1, batches.get(node1).size());
        assertEquals("Node1 should only have one batch for partition 0.", tp1, batches.get(node1).get(0).topicPartition);
    }
    ...
    public void testFlush() throws Exception {
        ...
        Map<Node, List<ProducerBatch>> results = accum.drain(cluster, result.readyNodes, Integer.MAX_VALUE, time.milliseconds());
        assertTrue(accum.hasIncomplete());
    ...
    public void testAbortIncompleteBatches() throws Exception {
        ...
        Map<Node, List<ProducerBatch>> drained = accum.drain(cluster, result.readyNodes, Integer.MAX_VALUE, time.milliseconds());
        assertTrue(accum.hasUndrained());
        assertTrue(accum.hasIncomplete());

        int numDrainedRecords = 0;
        for (Map.Entry<Node, List<ProducerBatch>> drainedEntry : drained.entrySet()) {
    ...
    public void testAbortUnsentBatches() throws Exception {
        ...
        RecordAccumulator.ReadyCheckResult result = accum.ready(cluster, time.milliseconds());
        assertFalse(result.readyNodes.isEmpty());
        Map<Node, List<ProducerBatch>> drained = accum.drain(cluster, result.readyNodes, Integer.MAX_VALUE,
                time.milliseconds());
        assertTrue(accum.hasUndrained());
        assertTrue(accum.hasIncomplete());
    ...
    public void testExpiredBatches() throws InterruptedException {
        ...
        assertEquals("Our partition's leader should be ready", Collections.singleton(node1), readyNodes);
        Map<Node, List<ProducerBatch>> drained = accum.drain(cluster, readyNodes, Integer.MAX_VALUE, time.milliseconds());
        assertEquals("There should be only one batch.", drained.get(node1).size(), 1);
        time.sleep(1000L);
        accum.reenqueue(drained.get(node1).get(0), time.milliseconds());
    ...
    public void testMutedPartitions() throws InterruptedException {
        ...
        accum.mutePartition(tp1);
        Map<Node, List<ProducerBatch>> drained = accum.drain(cluster, result.readyNodes, Integer.MAX_VALUE, time.milliseconds());
        assertEquals("No batch should have been drained", 0, drained.get(node1).size());

        // Test drain without muted partition.
        accum.unmutePartition(tp1, 0L);
        drained = accum.drain(cluster, result.readyNodes, Integer.MAX_VALUE, time.milliseconds());
        assertTrue("The batch should have been drained.", drained.get(node1).size() > 0);
    }
/*
