	 ...
	 private void sendProduceRequest(Node node, List<ProducerBatch> batches) {
		 ...
		 for (ProducerBatch batch : batches) {
            ...
            batchesByPartition.put(new TopicPartition(tp.topic(), tp.partition()), batch);*/
			 ...
			 ProduceRequest.Builder builderRequest = new ProduceRequest.Builder(batch.topicPartition, batch.records(), (short)1, -1, checkForDuplicate, retryMsgIdList);
			 ClientRequest request = client.newClientRequest(node, builderRequest, time.milliseconds(), true, -1, callback);
			 send(request, batch);  
		 }
	 }
	 ...
	 public void send(ClientRequest request, ProducerBatch batch)  {
		 ...
		 if(response.wasDisconnected()) {
			 ...
			 if(partitionResponse.getCheckDuplicate())
			 {
				 ...
				 //If first record is published successfully then entire batch would be published successfully. 
