...
import org.oracle.okafka.common.errors.RecordBatchTooLargeException;
...
public final class ProducerBatch {
    ...
    public void abort(RuntimeException exception) {
        ...
        completeFutureAndFireCallbacks( null, null, exception);
    }
     ...
     */
    public boolean done(List<String> msgIds, List<Long> logAppendTime, Exception exception) {
        final FinalState finalState;
        if (exception == null) {
            log.trace("Successfully produced messages to {} ", topicPartition);
    ...
    private void completeFutureAndFireCallbacks(List<String> msgIds, List<Long> logAppendTime, Exception exception) {
        // Set the future before invoking the callbacks as we rely on its state for the `onCompletion` call
        produceFuture.set(msgIds, logAppendTime, exception);
    ...
    public Deque<ProducerBatch> split(int splitBatchSize) {
        ...
        produceFuture.set( null, null, new RecordBatchTooLargeException());
        produceFuture.done();
