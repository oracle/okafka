...
public class TxEQAssignor extends AbstractPartitionAssignor {
	...
	public Map<String, List<TopicPartition>> assign(Map<String, Integer> partitionsPerTopic,
		...
		//System.out.println("TxEQAssignor: TxEQ Assign 1:Creating assignment map for each member ");
		for (String memberId : subscriptions.keySet())
		{
			//System.out.println("TxEQAssignor 2:MemberID in Assignment Map " + memberId);
			assignment.put(memberId, new ArrayList<TopicPartition>());
		...
		for(String topicNow : partitionsPerTopic.keySet())
		{
			...
			//System.out.println("TxEQAssignor 2.1: Total members to be assigned partitions are " + membersCnt);
			//int partitionCnt = partitionsPerTopic.get(topicNow);

			//Consider total partitions actually created instead of partitions which are supposed to be created
			// TxEQ creates partitions as and when message is published into the topic partitions.
			int partitionCnt = totalPartitionsCreated;
			
			int partForAll = partitionCnt/membersCnt;
			int partForSome = partitionCnt%membersCnt;
			//System.out.println("TxEQAssignor 2.2: Partitions for All "  + partForAll + " Partitions for Some " + partForSome);
			...
			/*
			 * Some sessions will get 1 additional partition.
			 * Below logic decides which sessions should be assigned 1 more partition. 
			 * Ideally, pick sessions which can be assigned partition locally.
			 */
			if(partForSome > 0 )
			{
				...
				for(SessionData sDataNow : memberList)
				{
					...
					instSDataList.add(sDataNow);
					//System.out.println("TxEQAssignor: Session count for inst " + instNow + " = " + instSDataList.size());
				...
				for(int instNow : instPListMap.keySet())
				{
					...
					int extraNow = (sessionCnt == 0 || pCnt == 0 )? 0 : Math.min(sessionCnt, moreForThisInst< 0 ? 0:moreForThisInst);
					extraPerInst[instNow-1] = Math.min( partForSome, extraNow);
					 
					//System.out.println("TxEQAssignor: Extra for Inst " + instNow + "= [" + extraPerInst[instNow-1] + "," + extraPerInstMax[instNow-1]+"]");
				...
				//Rest of the <partForSome> partitions can be distributed among any sessions as they will be assigned remotely
				...
				/*System.out.println("TxEQAssignor: Actual Extra per Partition ");
				for(int i =0 ; i<maxInst ; i++ )
				{
					System.out.println("TxEQAssignor: Inst: " + (i+1) + " Min Extra " + extraPerInst[i] +" Additional Extra " + extraPerInstMax[i] );
				}
			    // Assign one additional partition to a session 
				System.out.println("TxEQAssignor: Before actual assignment, Final pending partition count per session:"); */
				for(SessionData tMem : memberList)
				{
					...
					//System.out.println("TxEQAssignor: Session " + tMem.name + " To be assigned " + tMem.pendingCnt);
				}
			}
			...
			while(assigner.hasNext())
			{
				...
				//System.out.println("TxEQAssignor: Member " + tMem.name + " Pending partitions " + tMem.pendingCnt);
				...
				if(previousPartitions != null && previousPartitions.size() > 0)
				{
					for(PartitionData partNow : previousPartitions)
					{
						...
						//Check all partitions because due to instance shutdown a remote partition may have moved to local instance
						...
						if(pIndex!= -1)
						{
							...
							//System.out.println("Partitoin " +partNow.getTopicPartition().partition() + " assigned to " + tMem.name +" Pending " + tMem.pendingCnt);
						}
					}
			...
			while(assigner.hasNext())
			{
				...
				for(Integer pNow : localPartitionList)
				{
					...
					PartitionData teqP = new PartitionData(topicNow, tMem.getQueueId(), pNow, tMem.getSubscriberName(), tMem.getSubscriberId(),tMem.getInstanceId(), true);
					tMem.addAssignedPartitions(teqP);
					tMem.pendingCnt--;
					assignedNow.add(pNow);
					//System.out.println("TxEQAssignor: Partitoin " +pNow + " assigned to " + tMem.name +" Pending " + tMem.pendingCnt);
				}
			...
			while(assigner.hasNext())
			{
				...
				if(previousPartitions != null && previousPartitions.size() > 0)
				{
					...
					for(PartitionData pNow : previousPartitions)
					{
						...
						int pIndex = remotePartitionList.indexOf(pNow.getTopicPartition().partition());
						if(pIndex == -1)
						{
			...
			while(assigner.hasNext())
			{
				...
				for(Integer instnow : instPListMap.keySet())
				{
					...
					for(Integer pNow : remotePartitionList)
					{
						...
						PartitionData teqP = new PartitionData(topicNow, tMem.getQueueId(),  pNow.intValue(), tMem.getSubscriberName(), tMem.getSubscriberId(), instLookup, false);
						tMem.addAssignedPartitions(teqP);
						tMem.pendingCnt--;
			...
			while(assigner.hasNext())
			{
				...
				{
					PartitionData teqP = new PartitionData(topicNow, tMem.getQueueId(),  -1, tMem.getSubscriberName(), tMem.getSubscriberId(), tMem.getInstanceId(), true);
				...
				for(PartitionData teqPNow : assignedPartitionList)
				{
					...
					//System.out.println("TxEQAssignor:        " + teqPNow.getTopicPartition().partition());
				}
				previousPartitions.clear();
